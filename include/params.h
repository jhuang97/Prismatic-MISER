...
namespace Prismatic{
	...
	template <class T>
	struct PotWorkspace2D {
		Array1D<T> z;
		Array1D<T> zPlane;
		Array3D<T> potentialLookup;
	};

	template <class T>
	struct PotWorkspace3D {
		Array1D<T> z;
		Array1D<T> zvec;
		Array1D<T> zr;
		long numPlanes;
		Array4D<std::complex<T>> potLookup;
		Array2D<T> rband;
		Array2D<T> qband;
		Array2D<std::complex<T>> qxShift;
		Array2D<std::complex<T>> qyShift;
	};

	enum class pw_type {p2D, p3D};
	...
	struct PotentialWorkspace {
		...
		Array1D<T> sigma;
		Array1D<T> occ;
		std::map<size_t, size_t> Z_lookup;
		Array1D<long> xvec;
		Array1D<long> yvec;
		long dim0;
		long dim1;
		std::vector<size_t> unique_species;
		Array3D<T> extra_potential;
		std::vector<size_t> ep_imported_index;
		std::vector<T> ep_tiled_zindex;
		Prismatic::PotWorkspace2D<T> pw2D;
		Prismatic::PotWorkspace3D<T> pw3D;
		pw_type t;
	};
	...
	void Parameters<T>::calculateFileSize(){
		...
		if(meta.algorithm == Algorithm::Multislice || meta.algorithm == Algorithm::MS_MISER)
		{

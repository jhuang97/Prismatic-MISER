...
void setup4DOutput(Parameters<PRISMATIC_FLOAT_PRECISION> &pars)
{
	...
	if (pars.meta.algorithm == Algorithm::Multislice || pars.meta.algorithm == Algorithm::MS_MISER)
	{
	...
	for (auto n = 0; n < pars.numLayers; n++)
	{
		...
		if (pars.meta.algorithm == Algorithm::MS_MISER) {
			//create dataset for variance
			H5::DataSpace var_mspace(4, data_dims); //rank is 4
			H5::DataSet var_CBED_data;
			var_CBED_data = CBED_slice_n.createDataSet("variance", PFP_TYPE, var_mspace, plist);
			var_mspace.close();
		}
...
void save_complex_2D(Parameters<PRISMATIC_FLOAT_PRECISION> &pars, Array2D<std::complex<PRISMATIC_FLOAT_PRECISION>> arr, 
	...
	hsize_t mdims[2] = {arr.get_dimj(), arr.get_dimi()}; // no transposing
	// H5::Group testGroup(pars.outputFile.createGroup(addr));
	H5::Group testGroup = pars.outputFile.openGroup(addr);
	writeComplexDataSet_inOrder(testGroup, dsetname, &arr[0], mdims, 2);
}
...
void save_real_2D(Parameters<PRISMATIC_FLOAT_PRECISION> &pars, Array2D<PRISMATIC_FLOAT_PRECISION> arr, 
	...
	// H5::Group testGroup(pars.outputFile.createGroup(addr));
	H5::Group testGroup = pars.outputFile.openGroup(addr);
	writeRealDataSet_inOrder(testGroup, dsetname, &arr[0], mdims, 2);
}

void save_real_3D(Parameters<PRISMATIC_FLOAT_PRECISION> &pars, Array3D<PRISMATIC_FLOAT_PRECISION> arr, 
	const std::string &addr, const std::string &dsetname) {
	hsize_t mdims[3] = {arr.get_dimk(), arr.get_dimj(), arr.get_dimi()}; // no transposing
	// H5::Group testGroup(pars.outputFile.createGroup(addr));
	H5::Group testGroup = pars.outputFile.openGroup(addr);
	writeRealDataSet_inOrder(testGroup, dsetname, &arr[0], mdims, 3);
}

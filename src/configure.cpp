...
void configure(Metadata<PRISMATIC_FLOAT_PRECISION> &meta)
{
	...
	else if (meta.algorithm == Algorithm::MS_MISER) {
		std::cout << "Execution plan: MS_MISER\n";
		execute_plan = MS_MISER_entry;
	}
	else if (meta.algorithm == Algorithm::User) {
		// User put in code here for whatever
		std::cout << "Execution plan: User (not defined)\n";
		// execute_plan = User_entry;
	}
}
} // namespace Prismatic
